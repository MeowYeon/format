...
var require_formatter = __commonJS({
  "formatter.js"(exports2, module2) {
    ...
    async function loadMarkdownTools() {
      if (!markdownToolsPromise) {
        markdownToolsPromise = Promise.all([
          ...
          Promise.resolve().then(() => (init_unist_util_visit(), unist_util_visit_exports))
    ...
    async function formatText2(input) {
      const { processor, visit: visit2 } = await loadMarkdownTools();
      const tree = processor.parse(input);
      const plan = buildFormattingPlan(tree, visit2);
    ...
    function buildFormattingPlan(tree, visit2) {
      ...
      visit2(tree, "listItem", (node2, index2, parent) => {
        ...
        listItemsByLine.set(node2.position.start.line, {
    ...
    function applyLineShorthand(line) {
      ...
      const content3 = line.slice(markerIndex + 3);
    ...
    function replaceInlineBacktickShorthand(line) {
      ...
      while (index2 < line.length) {
        ...
        if (!inCodeSpan && line.startsWith(";;", index2)) {
          const closingIndex = line.indexOf(";;", index2 + 2);
          if (closingIndex !== -1) {
            const content3 = line.slice(index2 + 2, closingIndex);
            result += `\`${content3}\``;
            index2 = closingIndex + 2;
            continue;
          }
        }
        result += line[index2];
    ...
    function normalizeListItemLine(line, listItemInfo) {
      if (listItemInfo.ordered) {
        ...
        const marker2 = `${match2[2]}${match2[3]}`;
        const content4 = match2[5];
        if (content4.trim().length === 0) {
          return line;
        }
        return `${indent2}${marker2} ${content4.trimStart()}`.replace(/[ \t]*$/, "  ");
      }
    ...
    function summarizeChanges(before, after) {
      ...
      const changes = [];
      for (let index2 = 0; index2 < maxLength; index2 += 1) {
        const beforeLine = beforeLines[index2] ?? "";
        const afterLine = afterLines[index2] ?? "";
...
// main.js
var { Modal, Notice, Plugin, Setting, MarkdownView } = require("obsidian");
var { formatText } = require_formatter();
var MENU_TITLE = "Custom Format";
var PREVIEW_LIMIT = 8;
var FormatPreviewModal = class extends Modal {
...
module.exports = class FormatPlugin extends Plugin {
  async onload() {
    ...
    this.pendingCloseFormats = /* @__PURE__ */ new Set();
